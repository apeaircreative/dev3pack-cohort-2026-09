...
Your store reaches the instructor with `bootcamp submit ch10`. There is nothing
else to send. Run the lines in demo 10, so the file is at `demos/store.json` in
your course checkout. Running them in the session 10 notebook instead puts it
beside that notebook, and that works too. Keep one of the two, not both.

No `store.json` yet? Submit anyway. `bootcamp submit ch10` warns you and hands in
your notebook without it. Submit again once the file exists.
